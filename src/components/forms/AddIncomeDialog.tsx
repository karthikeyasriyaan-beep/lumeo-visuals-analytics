  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button size="sm" variant="outline" className="gap-1.5 sm:gap-2 shadow-lg hover:shadow-xl transition-all duration-300 border-success/30 hover:border-success/50 hover:bg-success/5 text-xs sm:text-sm h-9 sm:h-10 px-3 sm:px-4">
      ...
      <DialogContent className="sm:max-w-[425px] max-h-[90vh] border-border/50 bg-background/95 backdrop-blur-sm p-0">
        <DialogHeader className="p-4 sm:p-6 pb-0">
          <DialogTitle className="text-lg sm:text-xl font-semibold">Add Income</DialogTitle>
        </DialogHeader>
        <ScrollArea className="max-h-[calc(90vh-80px)] px-4 sm:px-6 pb-4 sm:pb-6">
          <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4 pt-4">
            <div className="space-y-1.5 sm:space-y-2">
              <Label htmlFor="source" className="text-xs sm:text-sm">Source</Label>
              <Input
                id="source"
            ...
            <div className="flex gap-2 sm:gap-3 pt-2">
              <Button type="button" variant="outline" onClick={() => setOpen(false)} className="flex-1 h-10 sm:h-11 text-sm">
                Cancel
              </Button>
